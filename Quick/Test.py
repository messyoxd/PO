...
class SortAlgorithmTester:
    ...
    def plot_data(self, data_list, title, ylabel, xlabel, workers=None) -> None:
        ...
        if workers:
            ...
            ax.legend()
            fig.savefig(f'QuickSort{workers}Threads.png')
        else:
            for i in range(len(data_list)):
                self.plot_in_lines(data_list[i][0], ax, data_list[i][1])
            ax.legend()
            fig.savefig(f'QuickSortSingleThread.png')
        ######################################
    ...
    if len(sys.argv) > 3 or len(sys.argv) < 2:
        ...
        if paralelizado:
            ...
            tester.plot_data(
                ...
                f"Comparacao QuickSort MultiThreaded ({workers} threads)",
            ...
            tester.plot_data(
                ...
                "Comparacao QuickSort Single Thread",
                "Tempo de computacao (segundos)",
                "Tamanho da instancia"
            )
            end = time.time()
